#!/bin/bash
#SBATCH --job-name=nanochat-climb-nm55
...
#SBATCH --output=runs/speedrun_climb_nm55_%j.log
#SBATCH --error=runs/speedrun_climb_nm55_%j.log

# Same as speedrun_slurm.sh but uses the climb_nm5.5 dataset instead of climbmix.
# Data must be pre-converted to parquet via: sbatch runs/convert_data.sh
...
export NANOCHAT_BASE_DIR=$LUSTRE_HOME/.cache/nanochat_climb_nm55
export NANOCHAT_DATA_DIR=$LUSTRE_HOME/.cache/nanochat/base_data_climb_nm55
...
# Tokenizer: train on climb_nm55 data (no download needed, data is local)
